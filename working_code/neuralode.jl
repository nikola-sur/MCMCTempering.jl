...
dudt2 = FastChain((x, p) -> x.^3, FastDense(2, 50, relu), FastDense(50, 2))
...
function l(θ)
    ...
    ll = sum(logpdf.(Normal.(ode_data, exp(θ[end]) + 1e-6), predict_neuralode(θ[1:end-1])))
    return lp + ll
end
function lp(θ)
...
function ll(θ)
    return sum(logpdf.(Normal.(ode_data, exp(θ[end]) + 1e-6), predict_neuralode(θ[1:end-1])))
end
...
prop = AdvancedHMC.NUTS{MultinomialTS, GeneralisedNoUTurn}(integrator; max_depth = 10)
adaptor = StanHMCAdaptor(MassMatrixAdaptor(metric), StepSizeAdaptor(0.5, prop.τ.integrator))
...
samples = sample(model, tempered(sampler, 4), 500; discard_initial=500)